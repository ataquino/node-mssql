...
const getTediousType = function (type, value) {
  switch (type) {
        ...
        if (this._isBatch) {
          if (Object.keys(this.parameters).length) {
            for (const name in this.parameters) {
              ...
              let value = getTediousType(param.type, param.value).validate(param.value)
          ...
          for (const name in this.parameters) {
            ...
            const param = this.parameters[name]
            if (param.io === 1) {
              req.addParameter(param.name, getTediousType(param.type, param.value), parameterCorrection(param.value), { length: param.length, scale: param.scale, precision: param.precision })
            } else {
              req.addOutputParameter(param.name, getTediousType(param.type, param.value), parameterCorrection(param.value), { length: param.length, scale: param.scale, precision: param.precision })
            }
          }
        }
